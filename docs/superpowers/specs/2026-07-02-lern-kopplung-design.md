...
**Zahlen:** Start-Seeding `SPAWN_INITIAL_DENSITY = 0.03` (3 % der passenden Biom-Zellen
tragen initial ein Objekt), Regeneration `SPAWN_RATE_PER_CELL_TICK = 1e-5`
(≈ 0.0024 Objekte je Biom-Zelle und Tag bei 240 Ticks/Tag; auf einem 200×200-Grid mit
~15 % Gebirge ≈ 14 neue Steine/Tag — Vorkommen versiegen nicht und fluten nicht).
...
**Konkreter Umschaltpunkt:** `Simulation.remove_dead()` (`simulation.py:251–259`) ist der
designierte, ursachen-agnostische Todes-Aggregationspunkt und ruft
`add_carcass(self.world, *agent.pos, CORPSE_ENERGY)`. Im v2-Modus ersetzt genau dieser
Aufruf sich durch die Objekt-Erzeugung. **Achtung — der Pfad ist heute real tot:**
`step()` filtert Tote VOR `remove_dead()` heraus (`simulation.py:493`,
`self.agents = [a for a in self.agents if a.alive]`; der TODO-Kommentar dort dokumentiert
es). Alle Update-Tode (Verhungern, Krankheit, Alter, Attack-Kill) erreichen
`remove_dead()` nie. Im v2-Modus entfällt dieses Pre-Filtering, damit Tote durch
`remove_dead()` laufen; bei Flag aus bleibt es unverändert (Golden). Ohne diesen Fix
entstünden im v2-Modus nie Kadaver — Massenleck im Ledger, und die Kern-Kette
Kadaver→Schneiden→Essen existierte nicht. Zusätzlich betroffen: der **Attack-Loot-Pfad**
...
Die log-prob hat damit **variable Struktur** (Slot-Term nur wenn ein Verb feuert und die
Menge nicht leer ist): jede Transition speichert die zulässige Maske und den gezogenen
Slot-Index mit, damit `evaluate_actions` beim PPO-Retraining exakt dieselbe
Konditionierung reproduziert. Die Kategorial-Verteilung bekommt einen eigenen
Entropy-Term in der 0.01-Gruppe (sonst kollabiert `W_sel` früh zu Quasi-Argmax, bevor
die Queries Bedeutung haben).
...
2. **`causal_epistemic`** — das Causal Model (unten) prädiziert **Verteilungen** (μ, σ per
   ...
   Rauschens: `max(0, mean(z²) − 1)` mit `z = (x − μ)/σ` — **Hinge außen**: für ein
   perfekt kalibriertes Modell ist E[mean(z²)] = 1, der Hinge schickt den Term gegen 0;
   ein Hinge innen (`mean(max(0, z²−1))`) würde dauerhaft ≈ 0.48 zahlen —
   skaleninvariante Rausch-Rente. Ein auskonvergierter, rein aleatorischer Prozess
   (Fragment-Zufall im `strike`) zahlt damit gegen 0 — ewiges Steineschlagen als
   Neugier-Farm ist zu. **Begrenzung:** log σ-Floor ≥ −2 auf der [0,1]-Prop-Skala (früh
   ist σ winzig → z² kann 100+ erreichen) und `curiosity` insgesamt geclampt auf
   [0, 2] (wie der bestehende Mechanismus).
...
- **Kadaver-Einmal-Münzung:** Tod im v2-Modus ⇒ genau ein Kadaver-Objekt mit `body_mass`,
  kein v1-Pool-Credit, kein Loot; gehaltene Objekte liegen am Boden. **Der Test läuft über
  `sim.step()`** (Agent stirbt im Tick), NICHT über direkten `remove_dead()`-Aufruf —
  sonst testet er den heute per Pre-Filtering umgangenen Pfad grün (B3).
...
- **Registry-Integrations-Smoke-Test:** `Simulation(physics_v2=True, headless=True)`,
  N Ticks ⇒ `"physics_v2_objects"` ist registriert und `world.objects` enthält gespawnte
  Objekte; Gegenprobe `physics_v2=False` ⇒ System wirkt nicht (Verwesung/Spawning laufen
  nicht nur als direkte Mechanik-Aufrufe, sondern nachweislich im Tick der Simulation).
...
- Kategoriale Auswahl: log-prob der Slot-Ziehung geht in die Policy-log-prob ein; leere
  ...
  log-prob-Summe (v2); **Retraining-Reproduktion:** für gespeicherte Transitionen liefert
  `evaluate_actions` mit Maske+Slot-Index bitgleiche log-probs wie zur Sampling-Zeit.

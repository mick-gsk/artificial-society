...
class Renderer:
    ...
    def draw_world(self, screen, world):
        ...
        for y in range(world.height):
            for x in range(world.width):
                ...
                ash  = cell['ash']
                if dis > 10:
                    screen.blit(self._alpha(cp, cp, (120, 40, 140, min(80, int(dis*1.2)))), (rx, ry))
                if pol > 15:
                    screen.blit(self._alpha(cp, cp, (80, 55, 20, min(70, int(pol*0.9)))), (rx, ry))
                if dist > 20:
                    screen.blit(self._alpha(cp, cp, (240, 120, 30, min(100, int(dist*1.3)))), (rx, ry))
                if ash > 12:
                    screen.blit(self._alpha(cp, cp, (100, 100, 100, min(90, int(ash*1.5)))), (rx, ry))
    ...
    def _draw_biome_texture(self, screen, biome, cell, gx, gy, cp, base_color):
        ...
        if biome == 'forest' and cp >= 8:
            for ox, oy in [(cp//4, cp//4), (cp*3//4, cp//3), (cp//2, cp*2//3)]:
                pygame.draw.circle(screen, detail, (rx+ox, ry+oy), max(2, cp//5))
        elif biome == 'grassland' and cp >= 8:
            for ox in (cp//4, cp//2, cp*3//4):
                h_t = max(2, cp//5)
                pygame.draw.line(screen, BIOME_DETAIL_COLOR['grassland'],
                                 (rx+ox, ry+cp-2), (rx+ox, ry+cp-2-h_t), 1)
        elif biome == 'desert' and cp >= 8:
            pygame.draw.arc(screen, BIOME_DETAIL_COLOR['desert'],
                            (rx+cp//6, ry+cp*2//3-cp//8, cp*2//3, cp//4), 0, math.pi, 1)
        elif biome == 'mountain' and cp >= 8:
            peak  = (rx+cp//2, ry+2)
            left  = (rx+2,     ry+cp-3)
            right = (rx+cp-2,  ry+cp-3)
            pygame.draw.polygon(screen, BIOME_DETAIL_COLOR['mountain'], [peak, left, right], 1)
            sl = (rx+cp//2-cp//6, ry+cp//3)
            sr = (rx+cp//2+cp//6, ry+cp//3)
            pygame.draw.polygon(screen, (235, 240, 245), [peak, sl, sr])
        elif biome == 'swamp' and cp >= 8:
            pygame.draw.ellipse(screen, (45, 75, 90),
                                (rx+cp//4, ry+cp//3, cp//2, cp//4))
        elif biome == 'water' and cp >= 8:
            wave_y = ry + cp//2
            for i in range(0, cp-3, max(3, cp//4)):
                pygame.draw.arc(screen, BIOME_DETAIL_COLOR['water'],
                                (rx+i, wave_y-cp//8, max(3, cp//4), cp//6), 0, math.pi, 1)

    def _draw_camp(self, screen, rx, ry, cp):
        mid, q = cp//2, cp//4
        pygame.draw.polygon(screen, (215, 185, 100),
                            [(rx+q, ry+cp-3), (rx+mid, ry+3), (rx+3, ry+cp-3)], 1)
        pygame.draw.polygon(screen, (215, 185, 100),
                            [(rx+mid, ry+cp-3), (rx+cp-3, ry+cp-3), (rx+cp*3//4, ry+4)], 1)

    def _draw_farm(self, screen, rx, ry, cp):
        step = max(3, cp//3)
        for i in range(rx+2, rx+cp-1, step):
            pygame.draw.line(screen, (160,130,60), (i, ry+2), (i, ry+cp-2), 1)
        for j in range(ry+2, ry+cp-1, step):
            pygame.draw.line(screen, (160,130,60), (rx+2, j), (rx+cp-2, j), 1)
    ...
    def draw_agents(self, screen, agents, tribes):
        ...
        for agent in agents:
            ...
            stage = getattr(agent, 'life_stage', STAGE_ADULT)

            # Radius by life stage
            ...
            if stage == STAGE_CHILD:
                ...
                for dx in (-3, 0, 3):
                    pygame.draw.line(screen, (240,210,100),
                                     (px+dx, py-radius-2), (px+dx, py-radius-6), 1)
                pygame.draw.line(screen, (240,210,100),
                                 (px-4, py-radius-2), (px+4, py-radius-2), 1)
    ...
    def _draw_vignette(self, screen):
        w, h, depth = self.dashboard_x, self.height, 22
        for i in range(depth):
            alpha = int(100 * ((1 - i/depth)**2))
            for (ex, ey), (ew, eh) in [
                ((0,0),(w,i+1)), ((0,h-i-1),(w,1)),
                ((0,0),(i+1,h)), ((w-i-1,0),(1,h)),
            ]:
    ...
    def draw(self, screen, world, agents, stats, tribes, technology):
        ...
        draw_dashboard(screen, self.dashboard_x, 0, 300, self.height,
                       ...
                       self.font, self.font_bold, self.small, tick=self._tick)
        pygame.display.flip()

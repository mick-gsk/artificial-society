...
class Renderer:
    ...
    def draw_world(self, screen, world):
        ...
        for y in range(world.height):
            for x in range(world.width):
                ...
                if dis > 10:
                    screen.blit(
                        self._alpha(cp, cp, (120, 40, 140, min(80, int(dis * 1.2)))), (rx, ry)
                    )
                if pol > 15:
                    screen.blit(
                        self._alpha(cp, cp, (80, 55, 20, min(70, int(pol * 0.9)))), (rx, ry)
                    )
                if dist > 20:
                    screen.blit(
                        self._alpha(cp, cp, (240, 120, 30, min(100, int(dist * 1.3)))), (rx, ry)
                    )
                if ash > 12:
                    screen.blit(
                        self._alpha(cp, cp, (100, 100, 100, min(90, int(ash * 1.5)))), (rx, ry)
                    )
                if cell["structures"].get("camp"):
                    self._draw_camp(screen, rx, ry, cp)
                if cell["structures"].get("farm"):
                    self._draw_farm(screen, rx, ry, cp)
                if cell["structures"].get("well"):
                    self._draw_well(screen, rx, ry, cp)
                if cp >= 10:
    ...
    def _draw_biome_texture(self, screen, biome, cell, gx, gy, cp, base_color):
        ...
        if biome == "forest" and cp >= 8:
            ...
            pygame.draw.arc(
                screen,
                BIOME_DETAIL_COLOR["desert"],
                (rx + cp // 6, ry + cp * 2 // 3 - cp // 8, cp * 2 // 3, cp // 4),
                0,
                math.pi,
                1,
            )
            ...
            pygame.draw.ellipse(
                screen, (45, 75, 90), (rx + cp // 4, ry + cp // 3, cp // 2, cp // 4)
            )
    ...
    def _draw_camp(self, screen, rx, ry, cp):
        ...
        pygame.draw.polygon(
            screen,
            (215, 185, 100),
            [(rx + q, ry + cp - 3), (rx + mid, ry + 3), (rx + 3, ry + cp - 3)],
            1,
        )
        pygame.draw.polygon(
            screen,
            (215, 185, 100),
            [(rx + mid, ry + cp - 3), (rx + cp - 3, ry + cp - 3), (rx + cp * 3 // 4, ry + 4)],
            1,
        )
    ...
    def draw_agents(self, screen, agents, tribes):
        ...
        for agent in agents:
            ...
            color = (
                tribes.color_for(agent.tribe_id)
                if agent.tribe_id is not None
                else agent.display_color()
            )
            ...
            pygame.draw.circle(
                screen, hi, (px - radius // 3, py - radius // 3), max(1, radius // 3)
            )
            # Outline
            ...
            if intensity > 20:
                ...
                pygame.draw.circle(
                    aura, (255, 230, 70, intensity), (radius * 3, radius * 3), radius + 8, 2
                )
    ...
    def draw_hotspots(self, screen, world):
        ...
        for x, y, cell in world.hotspots():
            if cell["disease"] > 25:
                pygame.draw.circle(
                    screen, (160, 40, 160), (x * cp + cp // 2, y * cp + cp // 2), max(3, cp // 2), 1
                )
            if cell["disturbance"] > 30:
                pygame.draw.rect(
                    screen, (240, 150, 40), (x * cp + 1, y * cp + 1, cp - 2, cp - 2), 1
                )

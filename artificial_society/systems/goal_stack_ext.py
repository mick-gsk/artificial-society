...
def _pottery_sequence(creator_id: int) -> GoalSequence:
    ...
    return GoalSequence(
        ...
        steps      = [
            SubGoal(action='collect', target_mat='clay',   max_ticks=15, label='collect_clay',
                    reward_pred=0.1),
            SubGoal(action='form',    target_mat=None,      max_ticks=10, label='form_clay',
                    reward_pred=0.3,
                    done_fn=lambda a, c: any(
                        getattr(o, 'shape', '') == 'hollow' for o in c.get('objects', []))),
            SubGoal(action='place_on_heat', target_mat=None, max_ticks=20, label='fire_pottery',
                    reward_pred=0.5,
                    done_fn=lambda a, c: any(
...
def _farming_sequence(seed_mat: str, creator_id: int) -> GoalSequence:
    ...
    return GoalSequence(
        ...
        creator_id = creator_id,
        steps      = [
            SubGoal(action='collect', target_mat=seed_mat, max_ticks=20, label=f'get_{seed_mat}',
                    reward_pred=0.2),
            SubGoal(action='plant',   target_mat=seed_mat, max_ticks=5,  label='plant',
                    reward_pred=0.4),
            SubGoal(action='wait',    target_mat=seed_mat, max_ticks=60, label='wait_growth',
                    reward_pred=0.0,
                    done_fn=lambda a, c: c.get('materials', {}).get(seed_mat, 0) > 0.5),
            SubGoal(action='collect', target_mat=seed_mat, max_ticks=10, label='harvest',
                    reward_pred=1.2),
        ]
    )
...
def _shelter_sequence(creator_id: int) -> GoalSequence:
    ...
    return GoalSequence(
        ...
        creator_id = creator_id,
        steps      = [
            SubGoal(action='collect', target_mat='dry_wood', max_ticks=20, label='collect_wood',
                    reward_pred=0.1),
            SubGoal(action='collect', target_mat='stone',    max_ticks=20, label='collect_stone',
                    reward_pred=0.1),
            SubGoal(action='arch',    target_mat=None,        max_ticks=15, label='build_dome',
                    reward_pred=1.5,
                    done_fn=lambda a, c: any(
...
class SequenceLibrary:
    ...
    def observe_and_learn(
        ...
        for seq_id in model_lib:
            if seq_id in self.sequences and seq_id not in getattr(observer_agent, 'known_sequences', set()):
                ...
                if seq.avg_reward() > 0.3:  # Nur gute Sequenzen uebertragen
                    ...
                    seq.times_shared += 1
                    print(f'[CULTURE] tick={tick} agent_{observer_agent.id} '
    ...
    def record_outcome(
        ...
        if len(outcomes) >= 3:
            ...
            if count >= 3:
                ...
                print(f'[RECIPE] Sequence {seq_id} consistently produces '

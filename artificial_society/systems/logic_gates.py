...
def detect_gates(world) -> list[Gate]:
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            ...
            in_bounds = [(nx, ny) for nx, ny in neighbours if 0 <= nx < width and 0 <= ny < height]
            conductive_nb = [
                (nx, ny)
            ...
            switch_nb = [(nx, ny) for nx, ny in in_bounds if _is_switch_cell(world, nx, ny)]
...
class GateNetwork:
        ...
        if tick % 10 == 0:
            ...
            for g in new_gates:
                if g.gate_id not in self.gates:
                    ...
                    print(
                        f"[GATE] New {g.gate_type}-Gate detected: {g.gate_id} "
    ...
    def summary(self) -> dict:
        return {
            ...
            "by_type": {
                t: sum(1 for g in self.gates.values() if g.gate_type == t) for t in GATE_TYPES
            },

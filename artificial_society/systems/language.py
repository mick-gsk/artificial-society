...
def make_token(creator_id: int, tick: int, x: int, y: int, pigment_vec: np.ndarray) -> Token:
    """
...
class TokenMemory:
    ...
    def record_use(self, token_id: str, context_vec: np.ndarray, reward: float):
    ...
    def best_token_for_context(self, context_vec: np.ndarray) -> Optional[str]:
        """
        ...
        for token_id, assoc in self.associations.items():
            ...
            sim = float(
                np.dot(context_vec, avg_ctx)
                / (np.linalg.norm(context_vec) * np.linalg.norm(avg_ctx) + 1e-8)
            )
    ...
    def check_convergence(
        ...
        for token_id in all_token_ids:
            users = [
                mem
                for mem in agent_memories
                if token_id in mem.associations and mem.associations[token_id].use_count >= 3
            ]
            if len(users) < 2:
                continue
            ...
            if avg_sim > 0.7 and share >= threshold:
                ...
                events.append(event)
                print(
                    f"[LANGUAGE] tick={tick} shared_symbol={token_id} "
...
def agent_mark(
    ...
    surface_ok = (
        ...
        or any(
            getattr(obj, "flags", lambda: {})().get("flat", False)

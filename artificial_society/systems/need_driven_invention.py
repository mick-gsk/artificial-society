...
def _select_action_by_need(
    ...
    strike_affinity = (
        (float(mat_a_vec[IDX["hardness"]]) + float(mat_b_vec_safe[IDX["hardness"]]))
        * 0.5
        * sharp_need
    )
    ...
    heat_src = max(
        float(mat_a_vec[IDX["heat_emission"]]), float(mat_b_vec_safe[IDX["heat_emission"]])
    )
    place_affinity = float(mat_a_vec[IDX["edibility"]]) * food_need * heat_src

...
def _patch_discovery_registry() -> None:
    ...
    def register(self, vector, discoverer_id: int = -1, tick: int = 0, recipe=None) -> str:
        result = original_register(
            self, vector, discoverer_id=discoverer_id, tick=tick, recipe=recipe
        )
...
def _social_learning_step_cached(agent: Agent, agents: list[Agent], tick: int) -> float:
    ...
    for other in nearby:
        ...
        if other_reward > 0.5 and random.random() < observe_chance:
            ...
            if seq:
                fidelity = (
                    social_learning_mod.FIDELITY_BASE
                    + social_learning_mod.FIDELITY_TRUST_BONUS * max(0.0, trust)
                )
        ...
        if trust > 0.5 and random.random() < social_learning_mod.TEACH_PROB:
            ...
            if seq and other_causal:
                other_causal.receive_transmitted(
                    seq,
                    fidelity=social_learning_mod.FIDELITY_BASE
                    + social_learning_mod.FIDELITY_TRUST_BONUS * trust,
                )
                reward += 0.06
...
def _maybe_mark_language(
    agent: Agent, cell: dict, tick: int, context_vec: np.ndarray, reward: float
) -> float:
...
def patched_update(
    ...
    self.hydration = max(
        0.0,
        min(100.0, self.hydration - hydration_loss + structure_mods.get("hydration_bonus", 0.0)),
    )
    self.health = max(
        0.0, self.health - mods.get("health_drain", 0.0) * structure_mods.get("disease_factor", 1.0)
    )
    ...
    inv_prob = agent_mod.INVENTION_BASE_PROB + agent_mod.INVENTION_CURIOSITY_MULT * self.genes.get(
        "curiosity", 0.5
    )

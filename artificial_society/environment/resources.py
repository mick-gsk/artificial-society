...
def apply_event(cell, event_strength):
    ...
    cell["moisture"] = clamp(
        cell["moisture"] - 12.0 * drought + 8.0 * storm - 18.0 * fire, 0.0, 100.0
    )
    cell["water"] = clamp(cell["water"] - 0.35 * drought + 0.45 * storm - 0.1 * fire, 0.0, 100.0)
    cell["plant_food"] = clamp(
        cell["plant_food"] - 2.4 * drought - 5.8 * fire - 3.6 * blight, 0.0, 180.0
    )
...
def diffuse_step(cell, neighbor_avgs):
    cell["pollution"] = clamp(
        0.92 * cell["pollution"] + 0.08 * neighbor_avgs["pollution"], 0.0, 100.0
    )
    ...
    cell["disturbance"] = clamp(
        0.9 * cell["disturbance"] + 0.1 * neighbor_avgs["disturbance"], 0.0, 100.0
    )
...
def regrow_cell(cell, biome, season_state, weather_state, tick, event_strength):
    ...
    cell["spoilage"] = clamp(
        cell["spoilage"] + decayed_meat * 0.85 - 0.05 * rain - 0.04 * cell["structures"]["camp"],
        0.0,
        100.0,
    )
    cell["disease"] = clamp(
    ...
    cell["water"] = clamp(
        cell["water"] + water_gain + 0.05 * cell["structures"]["well"], 0.0, 100.0
    )
    cell["moisture"] = clamp(
        cell["moisture"]

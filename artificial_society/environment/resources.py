...
# das 6-9-fache des ~0.01/Tick-Nachwuchses — und konnte kahle Zellen selbst ohne
# Verbrauch nicht mehr nachwachsen lassen. Die proportionale Form verschwindet
# bei plant_food -> 0, sodass Wachstum die Basis immer bis zu einem Gleichgewicht
# wieder aufbaut, statt sie zu sterilisieren.
...
def regrow_cell(world, x, y, biome, season_state, weather_state, tick, event_strength):
    ...
    world.set_cell(
        ...
        clamp(
            cell["plant_food"]
            + plant_gain
            - WIND_PLANT_LOSS * wind * cell["plant_food"]
            - 0.02 * cell["ash"],
            0.0,
            plant_hard_cap,
        ),
...
def regrow_grid(world, season_state, weather_state, tick, event_fields):
    ...
    F["spoilage"] = np.clip(spoilage + decayed_meat * 0.85 - 0.05 * rain - 0.04 * camp, 0.0, 100.0)
    F["disease"] = np.clip(
        fault0
    ...
    F["moisture"] = np.clip(F["moisture"] - 12.0 * drought + 8.0 * storm - 18.0 * fire, 0.0, 100.0)

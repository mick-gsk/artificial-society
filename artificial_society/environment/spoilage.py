"""
Spoilage & Biological Decay System
----------------------------------------
Organische Materialien (edibility > 0, dryness < 0.6) driften
über Zeit durch biologische Prozesse:

  Stage 0: frisch
  Stage 1: fermentiert  → Alkohol-analog (toxicity↑, trail↑, edibility±)
  Stage 2: gereift      → Essig/Käse-analog (toxicity stabilisiert, trail↑↑)
...
from artificial_society.environment.materials import DISCOVERY_REGISTRY, IDX


# ---------------------------------------------------------------------------
# Spoilage-Zustand eines Material-Lots
# ---------------------------------------------------------------------------
@dataclass
class SpoilageState:
    mat_id: str
    ...
    ticks_stored: int = 0
    stage: int = 0  # 0=fresh 1=spoiling 2=aged 3=rotten
...
def _apply_stage_drift(state: SpoilageState, stage: int) -> np.ndarray:
    """
    ...
    if stage == 1:  # Fermentierend: Alkohol entsteht
        ...
        vec[IDX["trail"]] = min(1.0, trail + 0.35)
        ...
        vec[IDX["trail"]] = min(1.0, trail + 0.55)  # intensiver Geruch
        ...
        vec[IDX["trail"]] = min(1.0, trail + 0.3)  # Verwesungsgeruch
...
def tick_spoilage(state: SpoilageState, env: dict) -> SpoilageState:
    """
    Einen Tick Spoilage berechnen.
    Gibt aktualisierten SpoilageState zurück.
    """
    if not _is_fermentable(state.current_vec):
        return state

    rate = _spoilage_rate(state, env)
    ...
    if effective_ticks > threshold and state.stage < 3:
        ...
        # Gib dem neuen Spoilage-Produkt eine ID in der Discovery Registry
        DISCOVERY_REGISTRY.register(
            new_vec,
            discoverer_id=-2,  # -2 = natürlicher Prozess
            tick=state.ticks_stored,
            recipe=(f"spoilage_stage{new_stage}", state.mat_id, None),
        )
...
def _is_fermentable(vec: np.ndarray) -> bool:
    ...
    return float(vec[IDX["edibility"]]) > 0.05 and float(vec[IDX["dryness"]]) < 0.85


# ---------------------------------------------------------------------------
# World-level Spoilage Manager
# ---------------------------------------------------------------------------
class SpoilageManager:
    """
    Verwaltet alle aktiven Spoilage-Prozesse in der Welt.
    Wird von invention.py pro Tick aufgerufen.
    """

    def __init__(self):
        # key: (cell_x, cell_y, mat_id) → SpoilageState
        self.states: dict[tuple, SpoilageState] = {}

    def register_material(self, x: int, y: int, mat_id: str, vec: np.ndarray, sealed: bool = False):
        key = (x, y, mat_id)
        if key not in self.states:
            self.states[key] = SpoilageState(
                mat_id=mat_id,
    ...
    def tick(self, world) -> list[dict]:
        """Tick alle aktiven Spoilage-Prozesse. Gibt Events zurück."""
        events = []
        to_remove = []
        for (x, y, mat_id), state in self.states.items():
            try:
                ...
                state = tick_spoilage(state, env)
                self.states[(x, y, mat_id)] = state

                if state.stage != old_stage:
                    events.append(
                        {
                            "type": "spoilage_stage_change",
                            "x": x,
                            "y": y,
                            "mat_id": mat_id,
                            "stage": state.stage,
...
SPOILAGE_MANAGER = SpoilageManager()

...
MATERIALS: dict[str, np.ndarray] = {
    ...
    "stone": _v(
        flammable=0.0, hardness=1.0, edibility=0.0, mass=1.0, dryness=0.5, conductivity=0.4
    ),
    "flint": _v(
        flammable=0.0,
        hardness=1.0,
    ...
    "seed_grain": _v(
        flammable=0.1, hardness=0.3, edibility=0.5, mass=0.05, dryness=0.7, solubility=0.15
    ),
    "leaf": _v(
        flammable=0.4,
        hardness=0.05,
    ...
    "water_drop": _v(
        flammable=0.0, hardness=0.0, edibility=0.3, mass=0.1, dryness=0.0, solubility=1.0
    ),
}
...
_LEGACY_FLAGS: dict[str, dict] = {
    ...
    "leaf": {"perishable": True},
    "seed_grain": {"plantable": True},
    "water_drop": {"liquid": True},
}
...
class DiscoveryRegistry:
    ...
    def register(
        ...
        print(
            f"[DISCOVERY] tick={tick} agent={discoverer_id} invented {new_id} "
        ...
        return [
            {
                ...
                "uses": e["uses"],
                "props": {
                    PROP_DIMS[i]: round(float(e["vector"][i]), 3)
...
# ---------------------------------------------------------------------------
def _thermal_reaction(vec_a: np.ndarray, vec_b: np.ndarray, env: dict) -> np.ndarray | None:
...
def _mechanical_reaction(vec_a: np.ndarray, vec_b: np.ndarray, action: str) -> np.ndarray | None:
    if action == "strike":
        if vec_a[IDX["hardness"]] > 0.6 and vec_b[IDX["hardness"]] > 0.6:
            result = vec_a.copy()
            result[IDX["sharpness"]] = min(
                1.0, (vec_a[IDX["hardness"]] + vec_b[IDX["hardness"]]) * 0.55
            )
...
def _solvent_reaction(vec_a: np.ndarray, vec_b: np.ndarray, env: dict) -> np.ndarray | None:
...
def apply_interaction(action: str, mat_a: str, mat_b: str | None, env: dict) -> list[str]:
    ...
    if action == "strike":
        ...
        if (
            props_a.get("sharp_when_knapped", 0) > 0.5
            and random.random() < props_a["sharp_when_knapped"]
        ):
        ...
        if heat > 0.5:
            if mat_a == "raw_meat" and random.random() < heat:
                ...
                props_a.get("flammable", 0) > 0.5 and random.random() < heat * props_a["flammable"]
            ):
        ...
        edibility = (
            props_a.get("edible_cooked", 0)
            if mat_a.startswith("cooked")
            else props_a.get("edible_raw", 0)
        )
        if not edibility:
            edibility = props_a.get("edibility", 0)
        if edibility > 0.3:
            results.append(
                "_nutrition:" + str(round(edibility + props_a.get("energy_bonus", 0.0), 2))
            )
...
def material_reward(vector: np.ndarray, agent_state: dict) -> float:
    ...
    # NOTE: hydration_deficit / health_deficit are not yet wired into the reward —
    # material_reward currently shapes only on energy/heat/light. Adding thirst/health
    # terms would change the golden trajectory, so it's a deliberate Phase-2 decision.
    ...
    utility = (
        ...
        + vector[IDX["heat_emission"]]
        + vector[IDX["light_emission"]]
    )
    if utility < 0.1:

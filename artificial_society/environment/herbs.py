...
def collect_herb(world, x, y, tag: str, amount: float = 1.0) -> float:
    """Remove up to `amount` of herb from the cell at (x, y); returns how much was taken."""
    cell = world.get_cell(x, y)
    herbs = cell.get("herbs", {})
    available = herbs.get(tag, 0.0)
    taken = min(available, amount)
    herbs[tag] = available - taken
    world.set_cell(x, y, "herbs", herbs)
    return taken

...
def _growth_factor(vec: np.ndarray, env: dict, season: str = "summer") -> float:
    ...
    # Jahreszeit
    season_factor = {"spring": 1.2, "summer": 1.0, "autumn": 0.6, "winter": 0.05}.get(season, 1.0)
    ...
    for y in range(height):
        for x in range(width):
            ...
            for mat_id, qty in list(slot.items()):
                ...
                factor = _growth_factor(
                    ext_vec[:N_PROPS] if ext_vec is not None else np.zeros(N_PROPS), env, season
                )
                ...
                if qty > 0.3 and random.random() < effective_growth * 0.15:
                    ...
                    if nslot.get(mat_id, 0.0) < 0.1:  # noch nicht vorhanden
                        nslot[mat_id] = 0.1
                        world.set_cell(nx, ny, "materials", nslot)
                        events.append(
                            {
                                "type": "seed_dispersal",
                                "from": (x, y),
...
def _get_growth_vector(mat_id: str) -> Optional[np.ndarray]:
    ...
    if (
        vec is not None
        and float(vec.sum()) > 0
        and float(vec[IDX["mass"]]) < 0.2
        and float(vec[IDX["edibility"]]) > 0.1
    ):
        ext = np.zeros(N_PROPS_EXT, dtype=np.float32)
        ext[:N_PROPS] = vec

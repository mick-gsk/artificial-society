...
Jede Dimension mapt auf einen visuellen Parameter:
  ...
  trail         → Pulsfrequenz (volatile = schneller Puls)
...
from artificial_society.environment.materials import IDX, N_PROPS
...
def vector_to_visual(vec: np.ndarray, cell_px: int = 24) -> VectorVisual:
    ...
    total_w = (
        ...
        + float(v[IDX["flammable"]])
        + 1e-6
    )
    hue = (hue / total_w) % 1.0

    # Saturation: feucht = saettig, trocken = gedaempft
    sat = 0.4 + float(v[IDX["dryness"]]) * (-0.2) + float(v[IDX["trail"]]) * 0.3
    ...
    alpha = int(
        255 * (0.4 + float(v[IDX["mass"]]) * 0.4 + (1.0 - float(v[IDX["solubility"]])) * 0.2)
    )
    ...
    # --- Puls: trail (volatile = schneller Puls) ---
    pulse_freq = float(v[IDX["trail"]]) * 2.0  # 0..2 Hz
...
def _object_glyph(obj) -> str:
    ...
    if obj.shelter > 0.3:
        return "\u2302"  # Haus
    if obj.comfort > 0.4:
        return "\u25ac"  # Bett (Rechteck)
    if obj.capacity > 0.3:
        return "\u25cb"  # Behaelter (Kreis leer)
    if obj.attack_bonus > 0.3:
        return "\u2191"  # Waffe (Pfeil)
    if obj.insulation > 0.3:
        return "\u2248"  # Isolation (Wellen)
...
def spoilage_to_visual(state, cell_px: int, tick: int) -> dict:

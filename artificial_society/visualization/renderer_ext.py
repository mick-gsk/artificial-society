...
Danach hat renderer_instance folgende neue Methoden:
    ...
    draw_spoilage(screen, world, spoilage_manager)
...
from artificial_society.environment.materials import DISCOVERY_REGISTRY, MATERIALS
...
# ---------------------------------------------------------------------------
def _draw_shape(
    screen, shape: str, cx: int, cy: int, r: float, color, alpha: int, outline_color=None
):
    if not _PYGAME:
        return
    r = max(2, int(r))
...
def _draw_glyph(screen, font, glyph: str, cx: int, cy: int, color, alpha: int):
        ...
        for x in range(world.width):
            ...
            for mat_id, qty in slot.items():
                ...
                if vis.aura_radius > 1:
                    _draw_aura(screen, cx_px, cy_px, vis.aura_radius * pulse, vis.aura_color)
                ...
                if cp >= 16 and font:
                    _draw_glyph(screen, font, vis.glyph, cx_px, cy_px, vis.glyph_color, vis.alpha)
        ...
        for x in range(world.width):
            ...
            for obj in objects:
                ...
                if draw_fn == "circle_outline":
                    ...
                    pygame.draw.rect(screen, color, (cx_px - r, cy_px - r // 2, r * 2, r))
                    ...
                    for i in range(3):
                        pygame.draw.rect(
                            ...
                            (cx_px - r + i * 2, cy_px - r // 2 + i * 3, r * 2 - i * 4, r // 2),
                            1,
                        )
                    ...
                    pygame.draw.line(screen, color, (cx_px - r, cy_px), (cx_px + r, cy_px), 1)
                    pygame.draw.line(screen, color, (cx_px, cy_px - r), (cx_px, cy_px + r), 1)

                elif draw_fn == "arc_dome":
                    pygame.draw.arc(
                        screen,
                        color,
                        (cx_px - r, cy_px - r, r * 2, r * 2),
                        0,
                        math.pi,
                        max(2, r // 3),
                    )
                ...
                if params["shelter_halo"]:
                    ...
                    pygame.draw.circle(h_surf, (100, 140, 255, 40), (r * 2, r * 2), r + cp // 3)
                    screen.blit(h_surf, (cx_px - r * 2, cy_px - r * 2))

                # Capacity-Ring (Behaelter-Anzeige)
                if params["capacity_ring"]:
                    pygame.draw.circle(screen, (200, 200, 255), (cx_px, cy_px), r + 3, 1)

                # Glyph
                if cp >= 14 and font:
                    _draw_glyph(screen, font, glyph, cx_px, cy_px, (255, 255, 255), 200)
...
def _draw_tokens(self, screen, world):
    ...
    for token_id, token in TOKEN_WORLD.tokens.items():
        ...
        shared = any(
            token_id in mem.associations and mem.associations[token_id].is_shared
        ...
        if shared:
            ...
            pygame.draw.circle(
                ring_surf, (255, 215, 0, 120), (pulse_r * 2, pulse_r * 2), pulse_r, 2
            )
...
# draw_spoilage
# ---------------------------------------------------------------------------
def _draw_spoilage(self, screen, world, spoilage_manager=None):
    ...
    if spoilage_manager is None:
        from artificial_society.environment.spoilage import SPOILAGE_MANAGER

        spoilage_manager = SPOILAGE_MANAGER

    for (x, y, mat_id), state in spoilage_manager.states.items():
        if state.stage == 0:
            continue
        cx_px = x * cp + cp // 2
        cy_px = y * cp + cp // 2
        params = spoilage_to_visual(state, cp, tick)
        ...
        for bx_rel, by_rel, br in params.get("bubbles", []):
            ...
            pygame.draw.circle(screen, (*params["color"], 180), (bx, by), max(1, int(br)))
        ...
        for x in range(world.width):
            ...
            for mat_id, qty in slot.items():
                ...
                if cp >= 18 and font and r > 4:
                    _draw_glyph(screen, font, params["glyph"], cx_px, cy_px, (255, 255, 255), 160)
...
def _draw_logic_gates(self, screen, gate_network=None):
    ...
    for gate_id, gate in gate_network.gates.items():
        for sx, sy in gate.switch_cells:
            ...
            # Label (A/O/N/L/W)
            if cp >= 16 and font:
                _draw_glyph(screen, font, params["label"], cx_px, cy_px, (255, 255, 255), 220)
...
def patch_renderer(renderer):
    ...
    renderer.draw_spoilage = types.MethodType(_draw_spoilage, renderer)

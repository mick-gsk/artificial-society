...
NEED_INVENTION_INTERVAL = 32
INVENTION_REWARD_SCALE = 1.5
INVENT_ACTION_THRESHOLD = 0.15
NEED_BONUS_SCALE = 0.35
...
def _ensure_new_fields(agent):
    ...
    if not hasattr(agent, 'brain') or agent.brain is None:
        agent.brain = Brain(plasticity=getattr(agent, 'genes', {}).get('plasticity', 1.0))
...
class Agent:
    ...
    def maybe_craft(self, world, technology, intensity):
        if self.is_sleeping or intensity <= 0.05:
            return 0.0
        cell = world.get_cell(*self.pos)
        craft_reward = 0.0
        if self.material_inventory.get('raw_meat', 0.0) > 0.1 or self.material_inventory.get('raw_root', 0.0) > 0.1:
            craft_reward += agent_try_cook(self, cell)
        if intensity > 0.35:
            env = {
            ...
            craft_reward += agent_try_invention(self, cell, env) * 0.35
        return craft_reward
    ...
    def update(self, world, agents, tick, season_state, weather_state, tribes, economy, technology):
        ...
        craft_reward = self.maybe_craft(world, technology, max(0.0, (action['invent'] + 1.0) * 0.5))
        ...
        inv_reward = 0.0
        need_bonus = 0.0
        invent_drive = max(0.0, (action['invent'] + 1.0) * 0.5)

        if invent_drive > INVENT_ACTION_THRESHOLD and self._need_inv_cooldown <= 0 and not self.is_sleeping:
            need_vector = compute_need_vector(self, cell)
            need_bonus = min(1.0, sum(max(0.0, float(v)) for v in need_vector) / max(1, len(need_vector)))
        ...
        elif invent_drive > 0.45 and action['explore'] > 0.1:
            legacy_inv_reward = agent_try_invention(self, cell, env)
            inv_reward += legacy_inv_reward * 0.4
            if legacy_inv_reward > 0:
                self.last_action_mode = 'experiment'
                self.endocrine.apply_discovery(min(1.0, legacy_inv_reward))
        ...
        reward = _homeostasis_reward(self.energy, prev_energy, self.hydration, prev_hydration, self.health, prev_health, self.alive)
        reward += herb_heal_reward + share_reward + craft_reward
        reward += inv_reward * INVENTION_REWARD_SCALE + need_bonus * NEED_BONUS_SCALE

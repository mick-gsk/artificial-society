...
class CommunicationSystem:
    ...
    def emit(self, agent, observation_vector, action=None):
        ...
        for i in range(self.message_size):
            ...
            mixed = 0.6 * src + 0.3 * act + noise * (1.0 - agent.traits["sociality"])
            base.append(max(-1.0, min(1.0, mixed)))
        agent.message_vector = base
        return base

    def evaluate_message_usefulness(
        self, sender, receiver, before_food, after_food, before_danger, after_danger
    ):
        score = 0.0
